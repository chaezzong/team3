...
CREATE TABLE review(
  ...
  FOREIGN KEY (payment_no) REFERENCES payment (payment_no)  ON DELETE CASCADE
);
...
VALUES(review_seq.nextval, 1, 3, 5, '댓글1', 5,  sysdate);
...
--  리뷰 번호로 조회
SELECT mem_nick, review_no, at_no, m_mem_no, review_word, review_rate, rdate
FROM (
...
FROM (
          SELECT m.mem_nick, m.mem_no as m_mem_no,
                      r.review_no, r.at_no, r.review_word, r.review_rate, r.rdate, r.mem_no as r_mem_no
          FROM mem m,  review r
            WHERE (m.mem_no = r.mem_no) AND r.payment_no=5
);


  SELECT mem_nick, review_no, at_no, m_mem_no, review_rate, review_word, rdate
  FROM (
              SELECT m.mem_nick, m.mem_no as m_mem_no,
                          r.review_no, r.at_no, r.review_word, r.review_rate, r.rdate, r.mem_no as r_mem_no
              FROM mem m,  review r
