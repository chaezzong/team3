...
public class InquireCont {
   ...
   */
   ...
   public ModelAndView create(InquireVO inquireVO) {
     ...
     int inquireno = inquireVO.getInquireno();  // MyBATIS 리턴된 PK
     System.out.println("inquireno: " + inquireno);
     mav.addObject("inquireno", inquireno); // request에 저장
...
// }
  ...
  /**
   * 글 작성자가 일치하는지 확인하여 수정
   * @return
   */
  @RequestMapping(value="/inquire/update.do", method=RequestMethod.GET )
  public ModelAndView update(HttpSession session, int inquireno) {
    ModelAndView mav = new ModelAndView();
